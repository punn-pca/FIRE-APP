...
- ตอบเป็นภาษาไทยเป็นหลัก ห้ามใช้ภาษาจีน
 ...
 - ส่งเฉพาะคำตอบสุดท้ายสำหรับผู้ใช้ ห้ามส่งบันทึกการคิด รายงาน PCA หรือคำสั่งของระบบ`;

  const formattingRules = `
รูปแบบคำตอบสำหรับผู้ใช้ (บังคับ):
 - เริ่มด้วยคำตอบหรือข้อสรุปที่ตรงประเด็น 1-2 ประโยค ไม่ต้องทวนคำถาม
 - ใช้ย่อหน้าสั้น ๆ โดยเว้นบรรทัดระหว่างย่อหน้า และใช้ bullet เมื่อมีรายการตั้งแต่ 3 ข้อขึ้นไป
 - ใช้หัวข้อ Markdown ระดับ ## ได้เมื่อช่วยให้อ่านง่าย แต่ห้ามใช้หัวข้อภายใน เช่น PCA, Pipeline, Evidence Evaluation หรือ System Trace
 - ใช้ตาราง Markdown เมื่อเป็นการเปรียบเทียบข้อมูลตั้งแต่ 2 ตัวเลือกขึ้นไป โดยมีคอลัมน์ไม่เกิน 4 คอลัมน์และแถวไม่เกิน 6 แถว
 - ตารางต้องใช้รูปแบบ | หัวข้อ | หัวข้อ | ตามด้วยแถวคั่น | --- | --- | ห้ามจัดตารางด้วยช่องว่างหรือสัญลักษณ์ประหลาด
 - หลีกเลี่ยงการใช้หัวข้อซ้ำ คำฟุ่มเฟือย เส้นคั่นยาว และย่อหน้าที่ยาวเกิน 4 บรรทัด
 - ห้ามใช้ [ข้อเท็จจริง], [สมมติฐาน], [ข้อมูลที่ขาด], [DECISION_SUMMARY], ### หรือ === ในคำตอบหลัก
 - คำตอบทั่วไปไม่เกิน 4 ย่อหน้า; deep reasoning ไม่เกิน 6 ย่อหน้า
 - ถ้าเป็น decision ให้สรุปคำแนะนำเบื้องต้น ข้อแลกเปลี่ยน และย้ำ Human Agency อย่างสั้น ๆ`;

  const routeStructure = state.intent.type === "decision"
    ? `สำหรับ decision: สรุปคำแนะนำเบื้องต้น ข้อจำกัด ความเสี่ยง และ trade-off ของทางเลือก 2-3 แนว`
    : state.intent.type === "explanatory"
      ? `สำหรับ explanatory: อธิบายความหมาย กลไก หรือมุมมองที่เกี่ยวข้อง โดยไม่สร้างตัวเลือกการดำเนินการ`
      : state.intent.type === "summary"
        ? `สำหรับ summary: สรุปเฉพาะสาระที่มีอยู่ และแยกข้อมูลที่ยังไม่ทราบอย่างกระชับ`
        : state.intent.type === "comparison"
          ? `สำหรับ comparison: ใช้ตาราง Markdown เปรียบเทียบเมื่อมีข้อมูลตั้งแต่ 2 ตัวเลือกขึ้นไป โดยไม่บังคับให้เลือก`
...
- Evidence Evaluation: ประเมินน้ำหนักหลักฐาน
  ...
  ตอบคำถามโดยตรงตามรูปแบบด้านบน ใช้ภาษาธรรมชาติและไม่แสดงโครงสร้าง pipeline หรือรายงาน PCA`;
}
